...
function normalizeRelPayload(rel) {
  // Accept array, or object with set/connect/disconnect
...
async function maybeDropRelationsOnPublishDuringCreate(event) {
  ...
  try {
    const existing = await strapi.db
      .query('api::pages-page.pages-page')
      .findMany({ where: { documentId } });
    if (Array.isArray(existing) && existing.length > 0) {
      // Draft exists; publishing creates a sibling. Avoid re-attaching relations here.
      if (data.quotes) delete data.quotes;
      if (data.festivalEvents) delete data.festivalEvents;
    }
  } catch (e) {
    // If anything goes wrong, fallback to safe normalization only.
